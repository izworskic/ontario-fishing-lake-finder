...
async function getJson(url,timeout=20000){
  const r=await fetch(url,{headers:{accept:'application/json','user-agent':'ChrisIzworskiOntarioFishingLakeFinder/4.1'},signal:AbortSignal.timeout(timeout)});
...
async function searchOhn(filters={},limit=100){
  ...
  const needGeometry=Number.isFinite(filters.originLat)&&Number.isFinite(filters.originLon)&&Number.isFinite(filters.maxDistanceKm);
  const page=await pageOhn({
    where:listWhere,
    ...spatial,
    outFields:OHN_FIELDS,
    returnGeometry:needGeometry?'true':'false',
    ...(needGeometry?{outSR:'4326',geometryPrecision:'5',maxAllowableOffset:'0.00005'}:{}),
    orderByFields:filters.q?'OFFICIAL_NAME_LABEL ASC':'SYSTEM_CALCULATED_AREA DESC'
  },Math.min(1000,Math.max(100,limit*2)));
  ...
  if(needGeometry) lakes=lakes.filter(x=>x.originDistanceKm!==null&&x.originDistanceKm<=filters.maxDistanceKm);
...
async function mapOhn(filters,bbox,maxMarkers=3000){
  const markerCeiling=Math.min(3000,Math.max(250,Number(maxMarkers)||3000));
  const base={where:ohnWhere(filters),...spatialForBbox(bbox)};
  const matchedInView=await countOhn(base);
  if(matchedInView>markerCeiling){
  ...
  const page=await pageOhn({...base,outFields:OHN_FIELDS,returnGeometry:'true',outSR:'4326',geometryPrecision:'5',maxAllowableOffset:'0.00005',orderByFields:'OBJECTID ASC'},Math.max(markerCeiling,matchedInView+1));
...
async function ohnLakeDetail(id){
  ...
  const d=await getJson(arcgisUrl({where:`OGF_ID=${ogf}`,outFields:OHN_FIELDS,returnGeometry:'true',outSR:'4326',geometryPrecision:'5',maxAllowableOffset:'0.00005',resultRecordCount:'2'}));
