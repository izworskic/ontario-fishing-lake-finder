...
const WATERBODY_FIELDS = 'OBJECTID,WATERBODY_IDENT,OFFICIAL_NAME,OFFICIAL_ALTERNATE_NAME,UNOFFICIAL_NAME,LATITUDE_DECIMAL_DEGREES,LONGITUDE_DECIMAL_DEGREES,GEOGRAPHIC_TOWNSHIP_NAME,UPPER_TIER_MUNICIPALITY,LOWER_TIER_MUNICIPALITY,SINGLE_TIER_MUNICIPALITY,TERRITORIAL_DISTRICT,LOCATION_NARRATIVE,REFRESH_DATETIME';
...
async function getJson(url, timeout=15000){
  const r = await fetch(url, {
    headers:{accept:'application/json','user-agent':'ChrisIzworskiOntarioFishingLakeFinder/2.0'},
...
async function pagedArcgis(base, params={}, options={}){
  ...
  while(offset<maxRecords){
    ...
    if(!d.exceededTransferLimit && page.length<count+1) break;
...
async function searchComplete(filters={},limit=100){
  ...
  let rows=pool.map(l=>{
    ...
    const item={...l,...loc,stocking:(stocks.get(l.id)||[]).slice(0,8)};
...
async function mapViewport(filters={},bboxValue,maxMarkers=12000){
  ...
  const spatial={geometry,geometryType:'esriGeometryEnvelope',inSR:'4326',spatialRel:'esriSpatialRelIntersects'};
  const [araPage,waterPage]=await Promise.all([
    ...
    pagedArcgis(SOURCES.waterbody,{
      where:'WATERBODY_IDENT IS NOT NULL',
