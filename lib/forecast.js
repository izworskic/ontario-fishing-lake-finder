...
function applyForecastToTrip(lake, forecast) {
  ...
  let adjustment = 0;
  const reasons = [];
  if (window.score >= 85) { adjustment = 6; reasons.push(`excellent upcoming 3-hour weather window (${window.score}/100)`); }
  else if (window.score >= 70) { adjustment = 3; reasons.push(`good upcoming 3-hour weather window (${window.score}/100)`); }
  else if (window.score < 40) { adjustment = -12; reasons.push(`poor upcoming 3-hour weather window (${window.score}/100)`); }
  else if (window.score < 55) { adjustment = -6; reasons.push(`limited upcoming 3-hour weather window (${window.score}/100)`); }
  ...
  return {
    ...
    tripScoreBreakdown: {
      ...
      reasons: [...(lake.tripScoreBreakdown.reasons || []), ...reasons].slice(0, 10),
...
module.exports = { CITYPAGE_ITEMS, langValue, normalizeHourly, hourScore, bestWindow, normalizeFeature, forecastContext, applyForecastToTrip };
